...
input_dir = os.path.join(scriptPath, "../downloads")
features = os.path.join(scriptPath, "features.csv")
...
def extract_audio_features(signal, sr):
    ...
    features = (
        [tempo]
        + list(np.mean(chroma_stft, axis=1))
        + list(np.mean(rmse, axis=1))
        + list(np.mean(spectral_centroid, axis=1))
        + list(np.mean(spectral_bandwidth, axis=1))
        + list(np.mean(rolloff, axis=1))
        + list(np.mean(zero_crossing_rate, axis=1))
        + list(np.mean(mfcc, axis=1))
        + list(np.mean(mfcc_delta, axis=1))
        + list(np.mean(tonnetz, axis=1))
        + list(np.mean(spectral_contrast, axis=1))
    )
...
with open(features, "a", newline="", encoding="utf-8") as file:
